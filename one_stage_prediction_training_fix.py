...
def description_prediction(embed_train, labels_train, embed_test):
    ...
    print(the_des_embed_train.shape)
...
labels = torch.load('./Results/descriptors_geode_continent_all/labels_all.pt', map_location=torch.device('cpu'))
filename_list = torch.load('./Results/descriptors_geode_continent_all/filename_list.pt', map_location=torch.device('cpu'))
predictions = torch.load('./Results/descriptors_geode_continent_all/predictions.pt', map_location=torch.device('cpu'))
predictions_all = torch.load('./Results/descriptors_geode_continent_all/all_predictions.pt', map_location=torch.device('cpu'))

label_to_classname = torch.load('./Results/descriptors_geode_continent_all/label_to_classname.pt', map_location=torch.device('cpu'))
...
#selected_idx = torch.load('./Results/descriptors_geode_gpt3/gpt4_des_top10_selected_idx.pt')
#load description scores, the length of the embedding should match the length of selectd index
#description_scores = torch.load('./Results/descriptors_geode_gpt3/gpt35_des_top10_two_stage_scores.pt')
...
train_idx = get_match_index(filename_list, './train_test_split/all_file_train.txt')
test_idx = get_match_index(filename_list, './train_test_split/all_file_test.txt')

print(len(train_idx), len(test_idx), len(filename_list))
the_embed_train = predictions_all[train_idx] 
the_embed_test = predictions_all[test_idx]

the_labels_train = labels[train_idx]   
the_labels_test = labels[test_idx]    

pred = description_prediction(the_embed_train, the_labels_train, the_embed_test)

acc_ori = accuracy_score(the_labels_test, descr_predictions[test_idx])
print('original accuracy:', acc_ori)
acc_des = accuracy_score(the_labels_test, pred)
print('prediction accuracy:', acc_des)


"""
class_pair_idx = [(2,26) , (33,38) , (4,16), (3,14), (22,24), (11,16), (24,28), (23,28), (1,14), (1,11)]

for pair in list(set(class_pair_idx)):
    ...
    train_idx = get_match_index(filename_list, trainfile)
    test_idx = get_match_index(filename_list, testfile)
    
    the_embed_train = predictions[train_idx] 
    the_embed_test = predictions[test_idx]
    ...
    acc_ori = accuracy_score(the_labels_test, descr_predictions[test_idx])
    acc_train = accuracy_score(the_labels_test,  pred)
